...
- **Sesiones 5-14 subidas a GitHub** (sesión 15) — 67 archivos llevaban ~3 meses sin commitear; el repo remoto estaba en el estado de mayo. Commit `158c6e7`. `.gitignore` ahora excluye `.claude/worktrees/` y `.claude/settings.local.json`
- **Keep-alive de Supabase** (sesión 15) — `.github/workflows/supabase-keepalive.yml`: query diario contra PostgREST para que el proyecto free no se pause por inactividad (se había pausado, causando que la app dejara de funcionar). Incluye un job que commitea un timestamp si el repo lleva 45+ días quieto, porque GitHub deshabilita los crons tras 60 días de inactividad del repo. Validado: run #1 verde
- **Backup semanal** (sesión 15) — `.github/workflows/supabase-backup.yml`: `pg_dump` de los schemas `public` y `auth` como artifact con 90 días de retención (el plan free de Supabase no incluye backups). El dump corre dentro de `postgres:17-alpine` porque el runner de Ubuntu trae `pg_dump` 16.14 y el servidor es 17.6. Guard de tamaño mínimo para detectar dumps vacíos. Validado: run #2 verde, artifact generado
- **Migración a publishable key** (sesión 15) — Supabase deprecia las keys JWT (`anon`/`service_role`) a fines de 2026; una app publicada con la key legacy dejaría de funcionar para todos los usuarios ya instalados. `supabase.ts` ahora tiene `SUPABASE_URL` y la publishable key como constantes; `app.config.js` dejó de inyectarlas. Las legacy siguen activas en el dashboard hasta validar en dispositivo
...
- **Publishable key hardcodeada, no por variable de entorno:** la publishable key está diseñada para viajar en el cliente — no otorga acceso por sí sola, lo que protege los datos es RLS. Tenerla como constante en `supabase.ts` elimina la causa raíz de BUG-005 en vez de parchearla: ya no hay variable de EAS que pueda llegar `undefined` en runtime. La *secret* key (`sb_secret_...`) nunca va al cliente, solo a Edge Functions.
- **No migrar a base de datos propia:** evaluado en sesión 15. Reproducir lo que da Supabase (Postgres gestionado, auth, RLS, Storage, Edge Functions) son 60-100 h iniciales más mantenimiento permanente, y escribir auth propio es una de las formas más confiables de tener un incidente de seguridad. La pausa del proyecto fue el plan free funcionando como está documentado, no una falla de Supabase. En vez de migrar: migraciones versionadas, backups propios, y acceso a datos concentrado en los Context — con eso migrar algún día es un fin de semana, no un trimestre.
