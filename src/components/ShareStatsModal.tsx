...
import { effectiveAmount, CURRENCY_ORDER, CURRENCY_SYMBOLS, formatAmountCurrency } from '../utils';
...
function pdfFilename(): string {
  const d = new Date();
  return `vera-balance-${d.getDate()}-${MONTHS_ES[d.getMonth()]}-${d.getFullYear()}.pdf`;
}
...
export function ShareStatsModal({ visible, onClose }: Props) {
  ...
  const topPersons = useMemo(() => {
    return persons
      .map(p => {
        ...
        const net = pd.reduce((s, d) =>
          s + (d.direction === 'me_debe' ? effectiveAmount(d) : -effectiveAmount(d)), 0);
        return { person: p, net, count: pd.length };
      })
      .filter(x => x.count > 0)
  ...
  function buildHtml(logoSrc: string | null): string {
    ...
    const personRows = topPersons.map(({ person, net }) =>
      `<div style="display:flex;align-items:center;gap:10px;margin-bottom:10px">
         ${avatarHtml(person)}
         <div style="flex:1;color:rgba(255,255,255,0.85);font-size:14px;font-weight:600;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${person.name}</div>
         <div style="font-size:14px;font-weight:700;flex-shrink:0;color:${net >= 0 ? '#2ED573' : '#F05B53'}">${net >= 0 ? '+' : ''}${formatAmountCurrency(Math.abs(net), 'ARS')}</div>
       </div>`
    ).join('');

    const topSection = topPersons.length > 0
      ? `<div style="border-top:1px solid rgba(255,255,255,0.08);padding-top:16px">
           <div style="color:rgba(255,255,255,0.35);font-size:10px;font-weight:700;letter-spacing:1.5px;margin-bottom:12px">TOP PERSONAS</div>
           ${personRows}
         </div>`
      : '';
...
<body>
  ...
  <div style="display:flex;align-items:center;gap:14px;margin-bottom:20px">
    ${logoHtml}
    <div>
      <div style="font-size:20px;font-weight:800;letter-spacing:-0.5px">Mi balance</div>
  ...
  <div style="display:flex;justify-content:center;gap:24px;align-items:center">
    <div style="text-align:center">
      <div style="font-size:22px;font-weight:700">${pendingDebts.length}</div>
    ...
    <div style="text-align:center">
      <div style="font-size:22px;font-weight:700">${paidDebts.length}</div>
      <div style="color:rgba(255,255,255,0.4);font-size:11px;margin-top:2px">saldadas</div>
    </div>
    <div style="width:4px;height:4px;border-radius:2px;background:rgba(255,255,255,0.2);margin-bottom:12px"></div>
    <div style="text-align:center">
      <div style="font-size:22px;font-weight:700">${persons.length}</div>
      <div style="color:rgba(255,255,255,0.4);font-size:11px;margin-top:2px">personas</div>
    </div>
  </div>

  ${topSection ? `<div style="height:1px;background:rgba(255,255,255,0.08);margin:16px 0"></div>${topSection}` : ''}
  ...
  async function handleShare() {
    ...
    try {
      ...
      await Sharing.shareAsync(destUri, {
        ...
        dialogTitle: 'Compartir mi balance Vera',
        UTI: 'com.adobe.pdf',
      });
    } catch (e) {
    ...
    <Modal visible={visible} transparent animationType="fade" onRequestClose={onClose}>
      <View style={styles.overlay}>
        ...
        <View style={styles.sheet}>
          ...
          <Text style={styles.sheetTitle}>Mi balance</Text>
