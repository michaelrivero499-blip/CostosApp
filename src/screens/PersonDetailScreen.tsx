...
export function PersonDetailScreen({ route, navigation }: Props) {
  ...
  const { persons, debts, addDebt, updateDebtStatus, updateDebt, updateDebtPhoto, deleteDebt, deletePerson, updatePersonAvatar, addPartialPayment } = useCostos();
  const { theme } = useTheme();
  const styles = useMemo(() => createStyles(theme), [theme]);

  const [showAddDebt,    setShowAddDebt]    = useState(false);
  const [showProModal,   setShowProModal]   = useState(false);
  const [proModalReason, setProModalReason] = useState<'persons' | 'debts' | 'voice'>('debts');
  ...
  const directionHint = useHint('debt_direction', 0);
  ...
  // Computaciones condicionales — seguras cuando maybePerson es undefined
  const personDebts  = maybePerson ? debts.filter(d => d.personId === personId) : [];
  ...
  const currencyNets = maybePerson ? getPersonNetByCurrency(personDebts) : [];
  ...
  if (maybePerson && initialDebtIdsRef.current === null) {
    initialDebtIdsRef.current = new Set(allDebts.map(d => d.id));
  }

  allDebts.forEach(debt => {
    if (!debtAnimMapRef.current.has(debt.id)) {
      const isInitial = initialDebtIdsRef.current?.has(debt.id) ?? false;
      debtAnimMapRef.current.set(debt.id, new Animated.Value(isInitial ? 0 : 1));
    }
  });

  // useEffect ANTES del early return — garantiza conteo de hooks constante
  useEffect(() => {
    if (!initialDebtIdsRef.current) return;
    const initialAnims = [...initialDebtIdsRef.current]
      .map(id => debtAnimMapRef.current.get(id))
      .filter((v): v is Animated.Value => v !== undefined);

    // Header: ease-out rápido desde arriba — sin rebote
    Animated.timing(headerAnim, {
      toValue: 1, duration: 250,
      easing: Easing.out(Easing.cubic), useNativeDriver: true,
    }).start();

    // Profile card: ease-out con leve escala, 70ms después
    Animated.sequence([
      Animated.delay(70),
    ...
    // FAB: escala limpia sin rebote — igual que Monai
    Animated.sequence([
      Animated.delay(220),
    ...
    // Deudas: stagger ease-out a partir de 140ms, 50ms entre cada una
    if (initialAnims.length > 0) {
      Animated.sequence([
        Animated.delay(140),
        Animated.stagger(55, initialAnims.map(anim =>
  ...
  function confirmDeletePerson() {
    ...
    deletePerson(personId);   // ← bug fix: antes no llamaba deletePerson
    navigation.goBack();
  }

  function openAddDebt() {
    const activeDebts = debts.filter(d => d.status === 'pendiente').length;
    if (!IS_PRO_USER && activeDebts >= FREE_DEBTS_LIMIT) {
      setProModalReason('debts');
  ...
  async function handleAddDebt(desc: string, amount: number, direction: DebtDirection, currency: Currency, dueDate?: string, notes?: string, photoUri?: string) {
    const newId = await addDebt(personId, desc, amount, direction, currency, dueDate, notes);
    if (newId && photoUri) await updateDebtPhoto(newId, photoUri);
    setNewestDebtId(newId);
    setTimeout(() => setNewestDebtId(null), 600);
  }

  async function handleEditDebt(desc: string, amount: number, direction: DebtDirection, currency: Currency, dueDate?: string, notes?: string, photoUri?: string) {
    if (!editTarget) return;
    await updateDebt(editTarget.id, desc, amount, direction, currency, dueDate, notes);
    if (photoUri) await updateDebtPhoto(editTarget.id, photoUri);
    setEditTarget(null);
  }
  ...
  async function handleSettleAll() {
    ...
    for (const debt of activeDebts) {
      await updateDebtStatus(debt.id, 'pagado');
    }
  }
  ...
  async function handleChangeAvatar() {
    ...
    if (!result.canceled) await updatePersonAvatar(person.id, result.assets[0].uri);
  }
    ...
    <SafeAreaView style={styles.safe}>
      ...
      <ScrollView style={styles.scroll} showsVerticalScrollIndicator={false}>
        <Animated.View style={{
          ...
          transform: [
            { translateY: profileCardAnim.interpolate({ inputRange: [0, 1], outputRange: [18, 0] }) },
            { scale: profileCardAnim.interpolate({ inputRange: [0, 1], outputRange: [0.97, 1] }) },
          ],
        }}>
          <View style={styles.profileCard}>
            <TouchableOpacity onPress={handleChangeAvatar} activeOpacity={0.85} style={{ position: 'relative' }}>
              {person.avatarUrl ? (
                <Image source={{ uri: person.avatarUrl }} style={[styles.avatar, { borderRadius: 32 }]} />
              ) : (
            ...
            {personDebts.length > 0 && (
              <View style={styles.miniStatsRow}>
                <View style={styles.miniStat}>
                  <Text style={[styles.miniStatNum, { color: theme.text }]}>{activeDebts.length}</Text>
                  <Text style={[styles.miniStatLabel, { color: theme.subtext }]}>activas</Text>
                </View>
                <View style={[styles.miniStat, { borderLeftWidth: StyleSheet.hairlineWidth, borderColor: theme.border }]}>
                  <Text style={[styles.miniStatNum, { color: '#2ED573' }]}>{paidDebts.length}</Text>
                  <Text style={[styles.miniStatLabel, { color: theme.subtext }]}>saldadas</Text>
                </View>
                <View style={[styles.miniStat, { borderLeftWidth: StyleSheet.hairlineWidth, borderColor: theme.border }]}>
                  <Text style={[styles.miniStatNum, { color: theme.text }]}>{personDebts.length}</Text>
                  <Text style={[styles.miniStatLabel, { color: theme.subtext }]}>total</Text>
                </View>
              </View>
            )}

            <View style={styles.profileActions}>
              <TouchableOpacity style={styles.whatsappBtn} onPress={() => setShowReminder(true)} activeOpacity={0.8}>
                <Text style={styles.whatsappBtnText}>Recordatorio</Text>
              </TouchableOpacity>
              {activeDebts.length > 1 && (
                <TouchableOpacity style={styles.settleAllBtn} onPress={() => setShowSettleAll(true)} activeOpacity={0.8}>
                  <Text style={styles.settleAllText}>✓ Saldar todo</Text>
                </TouchableOpacity>
              )}
            </View>
          </View>
        ...
        {directionHint.visible && (
          <HintTooltip
            icon="💡"
            text={'Me debe: esa persona te debe a vos. Le debo: vos le debés. El neto se calcula solo, separado por moneda.'}
            onDismiss={directionHint.dismiss}
            arrow="top"
          />
        )}
        ...
        {personDebts.length === 0 && (
          <View style={styles.emptyState}>
            ...
            <Text style={styles.emptyText}>Sin deudas con {person.name}</Text>
            <Text style={styles.emptySubtext}>Tocá + o el micrófono para agregar la primera</Text>
            <TouchableOpacity style={styles.emptyAddBtn} onPress={openAddDebt} activeOpacity={0.8}>
              <Text style={styles.emptyAddText}>+ Agregar deuda</Text>
            </TouchableOpacity>
          </View>
        )}
      ...
      {!showAddDebt && !showVoice && statusTarget === null && deleteTarget === null && editTarget === null && !showDeletePerson && (
        <Animated.View style={[styles.fabRow, { transform: [{ scale: fabScale }] }]}>
          <TouchableOpacity style={styles.fab} onPress={openAddDebt}>
      ...
      <StatusPickerModal
        ...
        onPartialPayment={(debt) => setPartialTarget(debt)}
      ...
      <PartialPaymentModal
        ...
        debt={partialTarget}
        onConfirm={(debtId, amount) => addPartialPayment(debtId, amount)}
      ...
      <ConfirmModal
        visible={showSettleAll}
        title={`Saldar todo con ${person.name}`}
        message={`Se marcarán como pagadas las ${activeDebts.length} deudas activas. Esta acción no se puede deshacer.`}
        confirmLabel="Saldar todo"
        onConfirm={handleSettleAll}
        onCancel={() => setShowSettleAll(false)}
      />

      <PhotoViewerModal uri={viewingPhoto} onClose={() => setViewingPhoto(null)} />

      <ReminderImageModal
        visible={showReminder}
        person={person}
        debts={activeDebts.filter(d => d.status === 'pendiente')}
...
// ─── Count-up number — firma visual tipo Monai ──────────────────────────────
...
function CountUpAmount({
  ...
  useEffect(() => {
    ...
    Animated.timing(anim, {
      toValue: value, duration: 520,
      easing: Easing.out(Easing.cubic), useNativeDriver: false,
    }).start();
    return () => anim.removeListener(id);
  }, [value]);

  return <Text style={style}>{formatAmountCurrency(Math.round(disp), currency)}</Text>;
}

// ─── Animated debt row ──────────────────────────────────────────────────────

function AnimatedDebtRow({
  debt, entranceAnim, isNew, isMarkingPaid, onPress, onLongPress, onDelete, onViewPhoto, formatDate,
}: {
  debt: Debt;
      ...
      <Animated.View style={[containerStyle, { position: 'relative' }]}>
        <TouchableOpacity style={styles.debtCard} onPress={onPress} onLongPress={onLongPress} activeOpacity={0.7}>
          ...
          <View style={styles.debtInfo}>
            ...
            {debt.notes ? (
              <Text style={styles.debtNotes} numberOfLines={2}>{debt.notes}</Text>
            ) : null}
            <View style={styles.debtMeta}>
              ...
              {debt.dueDate && debt.status !== 'pagado' && (() => {
                const due = new Date(debt.dueDate!);
                const now = new Date();
                const diffDays = Math.ceil((due.getTime() - now.getTime()) / (1000 * 60 * 60 * 24));
                const isOverdue = diffDays < 0;
                const isSoon = diffDays >= 0 && diffDays <= 3;
                const label = isOverdue ? `Venció hace ${Math.abs(diffDays)}d` : diffDays === 0 ? 'Vence hoy' : `Vence en ${diffDays}d`;
                const color = isOverdue ? '#F05B53' : isSoon ? '#FF9500' : '#8E8E93';
                return (
          ...
          <View style={styles.debtRight}>
            {(debt.paidAmount ?? 0) > 0 && !isPaid ? (
              <>
                <Text style={[styles.debtAmount]}>
                  {formatAmountCurrency(debt.amount - (debt.paidAmount ?? 0), debtCurrency)}
                </Text>
            ...
            {debt.photoUrl ? (
              <TouchableOpacity onPress={() => onViewPhoto(debt.photoUrl!)} activeOpacity={0.8}>
                <Image source={{ uri: debt.photoUrl }} style={styles.debtThumb} />
              </TouchableOpacity>
            ) : null}
          </View>
        </TouchableOpacity>
...
function createStyles(t: Theme) {
  return StyleSheet.create({
    ...
    debtNotes: { fontSize: 12, color: t.subtext, marginTop: 3, marginBottom: 4, fontStyle: 'italic' },
    debtRight: { alignItems: 'flex-end', marginLeft: 8, gap: 6 },
    debtAmount: { fontSize: 15, fontWeight: '700', color: t.text },
    amountPaid: { color: t.subtext, textDecorationLine: 'line-through' },
    debtOriginal: { fontSize: 10, color: t.subtext, textAlign: 'right' },
    debtThumb: { width: 48, height: 48, borderRadius: 8 },
    ...
    emptyText: { fontSize: 18, fontWeight: '600', color: t.text, marginBottom: 4, textAlign: 'center' },
    emptySubtext: { fontSize: 14, color: t.subtext, textAlign: 'center', marginBottom: 20 },
    emptyAddBtn: { backgroundColor: '#00C4A8', borderRadius: 20, paddingHorizontal: 24, paddingVertical: 10 },
    emptyAddText: { color: '#fff', fontSize: 14, fontWeight: '700' },
    // Mini stats in profileCard
    miniStatsRow: {
      flexDirection: 'row', borderRadius: 12, overflow: 'hidden',
      borderWidth: StyleSheet.hairlineWidth, borderColor: t.border,
      marginTop: 14, marginBottom: 8, width: '100%',
    },
    miniStat: { flex: 1, alignItems: 'center', paddingVertical: 10 },
    miniStatNum: { fontSize: 18, fontWeight: '800' },
    miniStatLabel: { fontSize: 10, fontWeight: '600', marginTop: 2 },
    // Profile actions row
    profileActions: { flexDirection: 'row', gap: 10, marginTop: 8, width: '100%' },
    settleAllBtn: {
      flex: 1, backgroundColor: '#2ED57318', borderRadius: 24,
      paddingVertical: 11, alignItems: 'center',
      borderWidth: 1, borderColor: '#2ED57340',
    },
    settleAllText: { color: '#2ED573', fontSize: 14, fontWeight: '700' },
    fabRow: {
    ...
    fabMic: {
      width: 60, height: 60, borderRadius: 30, backgroundColor: '#00C4A8',
      alignItems: 'center', justifyContent: 'center',
    ...
    whatsappBtnText: { color: '#FFFFFF', fontSize: 16, fontWeight: '700' },
  });
}
